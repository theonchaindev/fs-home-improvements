          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? "translateY(0)" : "translateY(30px)", transition: "all 0.7s ease 0.1s" }}>
              <nav className="flex items-center gap-2 text-sm mb-6">
                ...
                <span className="text-[#D9001B]">{title}</span>
              </nav>
              <div className="text-5xl mb-5">{icon}</div>
              <p className="text-[#D9001B] text-sm font-bold uppercase tracking-[0.2em] mb-3">{subtitle}</p>
      ...
      <section className="py-24 bg-white">
        <div ref={contentRef.ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16" style={{ opacity: contentRef.inView ? 1 : 0, transform: contentRef.inView ? "translateY(0)" : "translateY(30px)", transition: "all 0.6s ease" }}>
            <p className="text-[#D9001B] text-sm font-bold uppercase tracking-[0.2em] mb-3">Why Choose Our {title}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {benefits.map((b, i) => (
              <div
                key={b.title}
                className="bg-gray-50 rounded-xl p-7 border border-gray-100 hover:border-[#D9001B]/20 hover:shadow-lg transition-all duration-400 group"
              ...
              >
                <div className="w-10 h-10 bg-[#D9001B] rounded-lg flex items-center justify-center text-white font-black text-lg mb-4">
                  {i + 1}
                </div>
      ...
      <section className="py-24 bg-[#0A0A0A]">
        <div ref={optionsRef.ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16" style={{ opacity: optionsRef.inView ? 1 : 0, transform: optionsRef.inView ? "translateY(0)" : "translateY(30px)", transition: "all 0.6s ease" }}>
            <p className="text-[#D9001B] text-sm font-bold uppercase tracking-[0.2em] mb-3">Our Range</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {options.map((o, i) => (
              <div
                key={o.title}
                className="bg-[#1A1A1A] border border-white/10 rounded-xl p-6 hover:border-[#D9001B]/30 transition-all duration-300"
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-1.5 h-1.5 bg-[#D9001B] rounded-full" />
      ...
      <section className="py-24 bg-white">
        <div ref={faqRef.ref} className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16" style={{ opacity: faqRef.inView ? 1 : 0, transform: faqRef.inView ? "translateY(0)" : "translateY(30px)", transition: "all 0.6s ease" }}>
            <p className="text-[#D9001B] text-sm font-bold uppercase tracking-[0.2em] mb-3">FAQ</p>
          ...
          <div className="space-y-4">
            {faqs.map((f, i) => (
              <div
                key={f.q}
                className="border border-gray-100 rounded-xl p-7 hover:border-[#D9001B]/20 transition-colors"
              ...
              >
                <h3 className="font-bold text-gray-900 mb-3 flex items-start gap-3">
                  <span className="text-[#D9001B] font-black text-lg leading-none mt-0.5">Q</span>
                  {f.q}
                </h3>
      ...
      <section className="py-20 bg-[#D9001B]">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/contact" className="inline-flex items-center justify-center gap-3 bg-white text-[#D9001B] font-bold px-10 py-5 rounded hover:bg-gray-100 transition-colors text-lg">
